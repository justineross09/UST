...
import Sublayout from 'components/Layout/Mainlayout';
...
export default function Login() {
  ...
  const getData = async () => {
    ...
    await axios
      .get(`/api/requestForm/get-request-form-by-id/${id}`)
      .then((response) => {
  ...
  const changeStatus = async (status) => {
    ...
    await axios
      .patch(
        `/api/requestForm/update-request-form/${id}`,
        {
          status: status,
        },
      )
  ...
  const send = async () => {
    await axios
      .post('/api/requestForm/emailSent/',{
        emailAdd:emailAdd,
        status:status,
